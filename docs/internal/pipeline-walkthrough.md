...
This is a get-up-to-speed guide for `esme-pretrain`: the phases, the packages
that own them, and the two ideas that run through everything.

For the authoritative design and run details, see
[`architecture.md`](../architecture.md), [`status.md`](../status.md), and the
[214M run card](../run-cards/pretrain-214m-b200.md). This doc ties the code
together.

## One Line

Raw text in, a servable base model out. At 214M parameters, the whole LLM
lifecycle stays visible, fast, and cheap to run.
...
| ------------------ | ------------------------------------------------------- | --------------------------------- |
| `cli/`             | Parse `esme-pretrain <cmd>` and dispatch                | `cli/__init__.py`, `cli/parser.py` |
...
| `pretrain_run.py`  | Config contract; validates everything before spend      | `pretrain_run.py`                 |

Two pieces are the glue that connects the rest:

- `cli/` triggers each phase. Every README command is a subparser here.
- `pretrain_run.py` is the launch validator. It refuses to launch unless
...
There are two data paths with different jobs:
...
the tokenizer trainer, and post-run eval. That avoids contamination and survives
resume.
...
on each candidate tokenizer and reports loss, so the choice has a measurement
behind it.
...
The useful mental model is the **residual stream**: a running vector per position
that the embedding writes, and that each block *adds* to. It does not replace the
stream. That is why 30 blocks train stably: gradients flow straight down the
additions.
...
- **MLP** is `SwiGLU`. It processes each position independently.
